        ...
        with open('usuarios') as archivo:
            ...
            while registro:
                if formulario.usuario.data == registro[0] and formulario.password.data == registro[1]:
                    ...
                    b= """{% block caca %}"""
                    c="""{% endblock %}"""
                    a = """{% extends "base.html" %}<br>"""
                    try:

                        csvs = pd.read_csv("asd.csv")
                        if len(csvs) == 5:
                            html_file = open('templates/caca.html','w+')
                            html_file.write( str(a) +b+ csvs.to_html()+c)
                            html_file.close()
                        else:
                            return render_template('cantinv.html')
...
@app.route('/ultimasCompras')
def ultimascompras():
    csv = pd.read_csv("asd.csv")
    html_file = open('~/Documents/IFTS/supreme-happiness-master/templates/caca.html','w+')
    html_file = html_file.write(csv.to_html())
    html_file.close()
